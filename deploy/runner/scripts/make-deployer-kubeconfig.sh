#!/usr/bin/env bash
# Run ON devops-pc with an ADMIN kubeconfig. Creates the namespace-scoped
# ci-deployer identity (../rbac-deployer.yaml) and writes a kubeconfig for it
# to ../kubeconfig, which docker-compose.yml mounts read-only into the runners.
#
...
#   kubectl -n anmates delete secret ci-deployer-token && rerun this script
set -Eeuo pipefail
...
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT
kubectl -n "$NS" get secret ci-deployer-token -o jsonpath='{.data.ca\.crt}' | base64 -d >"$tmp/ca.crt"
...
kubectl config --kubeconfig "$new" set-context ci-deployer@onprem \
  --cluster onprem --user ci-deployer --namespace "$NS" >/dev/null
...
kubectl --kubeconfig "$new" -n "$NS" auth can-i patch deployments >/dev/null \
  || { echo "ci-deployer cannot patch deployments in ${NS}" >&2; exit 1; }
