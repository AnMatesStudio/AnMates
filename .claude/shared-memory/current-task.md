...
**Status (2026-06-10) — DESIGN: màn Khám phá load quán gần qua Google Places (New) + branch sync:** Brainstorming session (chưa code). Spec: `docs/specs/2026-06-10-discover-nearby-google-places.md`. Quyết định: provider **Google Places API (New)** `places:searchNearby` (ghi đè quyết định cũ "Goong"), **billing account VN chấp nhận throttle ~2400 Places req/ngày** (hợp lệ nhất), backend Go proxy + cache 10' + daily budget guard (≥2160 → fallback OSM), radius mở rộng dần 3km→10km. **⚠️ Branch:** local main==TECH-9==67630e1 đều CŨ (thiếu venue backend); đã **fast-forward merge `origin/main`→TECH-9 (→ef3ef2a)** sạch không conflict để có đủ venue backend + discover_view đầy đủ; 12 file media staged giữ nguyên (không commit). Research: VN trong Google Maps "Prohibited Territories", chặn theo quốc gia billing account (không phải toạ độ), "free" không thoát VN; free tier 03/2025 Pro=5k/tháng.

**IMPLEMENTED (2026-06-10) — code done, 1 GCP blocker.** Plan executed inline 10/10 task, 4 commit TECH-9 (97b4a17/07a024d/381212d/fdb77e3). BE: `GET /venues/nearby` + NearbyProvider (GooglePlacesProvider radius 3→10km + budget guard 2400/day + OSM fallback). FE: NearbyVenueService + discover_view dùng nearby (rating/open-now). Verify: go build+vet+test PASS, flutter test 2/2 + analyze 0 err + build web OK. **⚠️ Live blocker (GCP, không phải code):** Google trả **403 PERMISSION_DENIED** → user phải **enable "Places API (New)"** (khác legacy "Places API") + key restriction include Places API (New) + IP. **OSM fallback verified live** (8 quán Q1 sort đúng sau khi thêm User-Agent fix 406). Chưa push/PR. Next: user fix GCP 403 → re-run live. See sessions/2026-06-10-discover-nearby-google-places-design.md.
